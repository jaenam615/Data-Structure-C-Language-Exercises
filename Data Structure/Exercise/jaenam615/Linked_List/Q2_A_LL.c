...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    ...
    int position = 1;
    if (fixsize1 >= fixsize2){ //리스트1의 크기가 2의 크기보다 크다면
        while(ll2->size > 0){
            temp = findNode(ll2, 0); //리스트 2 가장 앞의 원소를 찾아서 임시로 저장 후 
            insertNode(ll1, position, temp->item); //해당 값을 리스트1에 넣고
            removeNode(ll2,0); //리스트2에서 삭제한다
            position = position +2; // 삽입 위치를 2 뒤로 한다 (번갈아가며 나오게)
        }
    }   
    else{
        while(fixsize1>0){
            temp = findNode(ll2, 0);
            insertNode(ll1, position, temp->item);
            removeNode(ll2, 0);
            position = position + 2;
            fixsize1 --;
        }
