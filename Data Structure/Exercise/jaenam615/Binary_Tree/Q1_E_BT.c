...
{
    if (tree1 == NULL && tree2 == NULL) return 1; // 둘 다 비어있는 경우에는 트리가 같음으로 1 반환
    else if (tree1 == NULL || tree2 == NULL) return 0; // 위의 경우는 제외됨으로 XOR형태가 만들어짐 = 즉 하나만 NULL임으로 0 반환
    else{
        if(tree1->item != tree2->item) return 0; 트리의 현재 노드 값들이 서로 다르면 즉시 0 반환
        else return identical(tree1->left, tree2->left)*identical(tree1->right, tree2->right); //곱셈의 형식으로 and 형태로 리턴하게 (재귀)
    }
